 ...
 *
 * 이런 경우처럼 세로로 읽을 때
 * 해당 자리의 글자가 없으면 읽지 않고
 * 그 다음 글자를 계속 읽는다.
 *
 * 그림 1의 다섯 번째 자리를 세로로 읽으면
 * D1gk로 읽는다.
 *
 * 그림 1에서 영석이가 세로로 읽은 순서대로
 * 글자들을 공백 없이 출력하면 다음과 같다.
 *
 * Aa0aPAf985Bz1EhCz2W3D1gkD6x
 *
 * 입력
 * 총 다섯 줄의 입력이 주어진다.
 * 각 줄에는 최소 1개, 최대 15개의 글자들이
 * 빈칸 없이 연속으로 주어진다.
 *
 * 주어지는 글자는
 * 영어 대문자 'A'부터 'Z',
